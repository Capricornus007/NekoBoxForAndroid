...
object Theme {
    ...
    fun apply(context: Context) {
        applyBase(context, getTheme())
    }

    fun applyDialog(context: Context) {
        applyBase(context, getDialogTheme())
    }

    private fun applyBase(context: Context, themeResId: Int) {
        context.setTheme(themeResId)
        // mod/original 共用同一基础主题资源，resId 不变时 setTheme 是 no-op，
        // 强制重刷基础主题以清除 app context 上可能残留的 original overlay
        context.theme.applyStyle(themeResId, true)
        if (usingOriginalStyle()) {
            context.theme.applyStyle(R.style.ThemeOverlay_SagerNet_Original, true)
        }
    }

    var currentThemeStyle = -1
    fun usingOriginalStyle(): Boolean {
        if (currentThemeStyle == -1) {
            currentThemeStyle = DataStore.themeStyle
        }
        return currentThemeStyle == 1
    }
