...
class TrafficChartActivity : AppCompatActivity() {
    ...
    private fun startMonitoring() {
        if (!DataStore.serviceState.connected) {
            ...
            binding.cardConnectionError.visibility = View.GONE
            binding.tvMonitorStatus.text = getString(R.string.traffic_monitor_waiting)
            binding.tvMonitorStatus.setTextColor("#E65100".toColorInt())
        ...
        binding.tvMonitorStatus.setTextColor("#059669".toColorInt())
    ...
    private fun connectTrafficWebSocket() {
        ...
        trafficWebSocket = client.newWebSocket(
            request,
                ...
                override fun onMessage(webSocket: WebSocket, text: String) {
                    ...
                    try {
                        ...
                        runOnUiThread {
                            ...
                            binding.speedDownText.text = Formatter.formatFileSize(
                                this@TrafficChartActivity,
                                down,
                            ) + "/s"
                            binding.peakUpText.text = getString(
                                R.string.traffic_peak,
                            ) + ": " + Formatter.formatFileSize(
                                this@TrafficChartActivity,
                                binding.trafficChart.peakUp,
                            ) + "/s"
                            binding.peakDownText.text = getString(
                                R.string.traffic_peak,
                            ) + ": " + Formatter.formatFileSize(
                                this@TrafficChartActivity,
                                binding.trafficChart.peakDown,
                            ) + "/s"
                        }
                    } catch (_: Exception) {
                    }
                    ...
                    if (isForeground) {
                        ...
                        runOnUiThread {
                            if (wsRetryCount >= 4) {
                                ...
                                binding.chartStatusHint.visibility = View.VISIBLE
                                binding.chartStatusHint.text = getString(R.string.traffic_connecting_api)
                            }
                        }
                        // Reconnect attempt
        ...
        try {
            ...
            client.newCall(req).execute().use { resp ->
                    ...
                    for (i in 0 until connArray.length()) {
                        ...
                        items.add(
                            ConnectionModel(
                                ...
                                rule = "Rule: $rule",
                                chains = if (chainsList.isNotEmpty()) {
                ...
                withContext(Dispatchers.Main) {
                    binding.cardConnectionError.visibility = View.GONE
                    binding.connectionsCountTitle.text = "${getString(
                        R.string.traffic_active_connections,
                    )} (${items.size})"
                    binding.connectionsTotalStats.text = getString(
                        R.string.traffic_total_updown,
                        Formatter.formatFileSize(this@TrafficChartActivity, upTotal),
                        Formatter.formatFileSize(this@TrafficChartActivity, downTotal),
                    )
    ...
    private fun getAppDisplayInfo(uid: Int, processPath: String, destPort: String): AppDisplayInfo {
        ...
        val defaultName = if (destPort == "53") {
            "系统 DNS 解析"
        } else if (uid == 0) {
            "系统核心服务"
        } else {
