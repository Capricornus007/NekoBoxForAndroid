 ...
 *
 * Shows the standard protocol link (vless://, ss://, ...) and the SN link (sn://) in one
 * dialog; when both are available a tab switcher redraws the code live. Bitmap encoding runs
 * off the main thread on the view lifecycle (local behaviour, kept over the upstream
 * synchronous encoder). The legacy single-url constructor keeps working for old callers.
 */
class QRCodeDialog() : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val stdUrl = args.getString(KEY_STD_URL)
            ?: if (legacyUrl?.startsWith("sn://") == false) legacyUrl else null
        val universalUrl = args.getString(KEY_UNIVERSAL_URL)
            ?: if (legacyUrl?.startsWith("sn://") == true) legacyUrl else null
        ...
        // Tab switches can finish their (off-thread) encode out of order; only the newest
        // request may touch the image view.
        var encodeGeneration = 0
        ...
        fun updateCode(url: String, isSn: Boolean) {
            ...
            val generation = ++encodeGeneration
            ...
            runOnLifecycleDispatcher {
                try {
                    ...
                    onMainDispatcher {
                        if (generation == encodeGeneration) ivQrcode.setImageBitmap(bitmap)
                    }
