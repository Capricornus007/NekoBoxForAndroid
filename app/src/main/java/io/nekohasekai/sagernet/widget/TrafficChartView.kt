...
class TrafficChartView @JvmOverloads constructor(
    ...
    private val upLinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = "#FFB74D".toColorInt() // Orange/Amber for Up
        strokeWidth = 5f
        style = Paint.Style.STROKE
    ...
    private val downLinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = "#4DD0E1".toColorInt() // Cyan/Blue for Down
        strokeWidth = 5f
        style = Paint.Style.STROKE
        strokeCap = Paint.Cap.ROUND
    }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (h > 0) {
            // 漸變按實際高度建一次，避免每幀分配（DrawAllocation）
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        gridPaint.color = if (isDark) "#26FFFFFF".toColorInt() else "#1F000000".toColorInt()
        textPaint.color = if (isDark) "#99FFFFFF".toColorInt() else "#99000000".toColorInt()
