                                    ...
                                    when (proxy["plugin"]) {
                                        ...
                                        "v2ray-plugin" -> {
                                            ssPlugin.apply {
                                                ...
                                                val muxOpt = opts["mux"]
                                                val muxInt = muxOpt?.toString()?.trim()?.toIntOrNull()
                                                when {
                                                    muxOpt == null -> {}
                                                    muxOpt == true -> add("mux=8")
                                                    muxOpt == false -> add("mux=0")
                                                    muxInt != null -> add("mux=$muxInt")
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    when (opt.key) {
                                        ...
                                        "xhttp-opts" -> if (bean.isVLESS && bean.type == "xhttp") {
                                            (opt.value as? Map<String, Any?>)?.also { xhttpOpts ->
                                                ...
                                                // The full mihomo xhttp-opts mapping (padding, headers, xmux and the
                                                // download-settings / alt-downloader block) lives in XhttpExtraConverter.
                                                val extra = XhttpExtraConverter.clashToSingBox(xhttpOpts)
                                                if (extra.isNotBlank()) {
                                                    bean.xhttpExtra = extra
                                                }
                                            }
                                        }
