...
int byedpi_embedded_main(int argc, char **argv) {
    ...
    optopt = 0;
    optarg = NULL;
    // byeDPI 的 main() 會讀 SS_PLUGIN_OPTIONS/SS_LOCAL_PORT 來相容 shadowsocks
    // 外掛模式：那樣會整個蓋掉我們注入的 argv，並把 laddr 改成外部指定的端口。
    // 行程環境變數不該影響內嵌實例，進 main 前先清掉。
    unsetenv("SS_PLUGIN_OPTIONS");
    unsetenv("SS_LOCAL_PORT");
