...
// Outbound 在 libcore 进程内跑一份 byeDPI 的 event loop，对外用 SOCKS5 私有
// socketpair 通信（不监听任何 IP 端口）。同一份 CLI 策略的多个 profile 共用
// 一个 runner，引用计数见 bridge.go。
...
// ensureStarted 延后到真正要 dial 时才拉起 runner：注册阶段不该启动 C 线程。
...
// privateHandshake 在 socketpair 的一端跑 SOCKS5 握手，对端是 byeDPI 的
// on_android_control 注入进去的那个 fd。
