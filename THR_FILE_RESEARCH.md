...
（解析脚本：[`roo_parse_thrbackup.py`](roo_parse_thrbackup.py)，`uv run roo_parse_thrbackup.py <文件>`）：
...
[`roo_parse_thrbackup.py`](roo_parse_thrbackup.py)（约 60 行，无第三方依赖，`uv run` 直接执行）。
